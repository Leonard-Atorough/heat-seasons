...
describe("Given a Seasons page", () => {
  // Grouped tests to share intent and reduce duplicated setup

  describe("When rendering seasons", () => {
  ...
  describe("When joining a season", () => {
  ...
  describe("When deleting a season", () => {
    // delete success and failure flows
  ...
  describe("When interacting with modals", () => {
    // modal open/close, create and update flows
    ...
    it("opens edit season modal when edit button is clicked", async () => {
      ...
      mockedUseSeasons.mockReturnValue({
        data: [createSeason({ id: "s1", name: "Season 1", status: "upcoming", startDate: date })],
        isLoading: false,
        error: null,
        refresh,
      });
    ...
    it("calls API and refreshes list when season is updated", async () => {
      const user = userEvent.setup();
      const date = new Date("2024-12-01");
      mockedUseSeasons.mockReturnValue({
        data: [createSeason({ id: "s1", name: "Old Name", status: "upcoming", startDate: date })],
        isLoading: false,
        error: null,
        refresh,
      });
      ...
      mockedUpdateSeason.mockResolvedValue(
        createSeason({ id: "s1", name: "Updated Name", status: "upcoming", startDate: date }),
      );
      ...
      await waitFor(() => {
        expect(mockedUpdateSeason).toHaveBeenCalledWith("s1", {
          name: "Updated Name",
          status: "upcoming",
